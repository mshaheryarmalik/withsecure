...
export function PhaseDetailsModal({ phase, onClose }: PhaseDetailsModalProps) {
  ...
  const getStepIcon = (step: Step) => {
    switch (step.status) {
      ...
      case 'completed':
        return (
          <div className="w-6 h-6 bg-gradient-to-br from-slate-500 to-slate-600 rounded-full flex items-center justify-center">
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="overflow-y-auto max-h-[calc(80vh-120px)] px-6 py-6">
            <div className="relative">
              {/* Timeline line */}
              <div className="absolute left-[11px] top-8 bottom-8 w-0.5 bg-slate-800"></div>
          ...
          <div className="sticky bottom-0 bg-black border-t border-slate-800 px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="text-xs text-slate-500 font-mono">
                {phase.steps.filter(s => s.status === 'completed').length} / {phase.steps.length} steps completed
              </div>
              <button
                onClick={onClose}
